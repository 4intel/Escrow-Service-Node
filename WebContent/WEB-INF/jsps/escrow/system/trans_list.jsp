...
<script>
	function searchAction() {
		...
		listForm.action="/index.do";
		listForm.target="_self";
		listForm.submit();
	}
	
	function nextStep(idx,p1,p2,p3,p4,p5) {
		listForm.idx.value = idx;
		listForm.pSWID.value = p1;
		listForm.pSWID_NM.value = p2;
		listForm.pPRICE.value = p3;
		listForm.pCONTENTS.value = p4;
		listForm.pBWID.value = p5;
		listForm.curl.value = "/escrow/system/trans_view.do";
		listForm.action="/index.do";
		listForm.target="_self";
		listForm.submit();
	}
	...
	<div class="container">
		...
		<input type="hidden" name="curl" id="curl" value="/escrow/system/trans_list.do" />
		<input type="hidden" name="pageIndex" id="pageIndex" value="${pageInfoVO.pageIndex }" />
		<input type="hidden" name="pSWID" value="" />
		<input type="hidden" name="pSWID_NM" value="" />
		<input type="hidden" name="pPRICE" value="" />
		<input type="hidden" name="pCONTENTS" value="" />
		<input type="hidden" name="pBWID" value="" />
		...
		<div class="col-lg-12">
			...
			<div class="jumbotron" style="padding-top: 20px;">
				...
				<div class="row">				    
					<table class="table table-striped" style="text-align: center; border:1px solid #ffffff">
						...
						<tbody id='id_accountlist_table_tbody'>
							...
							<tr style="background-color:#ffffff;">
								...
								<td>
									<c:if test="${escrowInfo.TSTATE==1}"><span style="color:blue;">구매요청</span></c:if>
									<c:if test="${escrowInfo.TSTATE==2}"><span style="color:blue;">배송출발</span></c:if>
									<c:if test="${escrowInfo.TSTATE==3}"><span style="color:blue;">입금대기</span></c:if>
									<c:if test="${escrowInfo.TSTATE==4}"><span>거래완료</span></c:if>
								</td>
								<td>
									<c:if test="${escrowInfo.TSTATE==1}">
										배송 준비중...
									</c:if>
									<c:if test="${escrowInfo.TSTATE==2}">수취확인 대기중...</c:if>
									<c:if test="${escrowInfo.TSTATE==3}"><button type="button" class="btn btn-success" onclick="nextStep('${escrowInfo.IDX}','${escrowInfo.SWID}','${escrowInfo.SWID_NM}','${escrowInfo.BOARD_PRICE}','${escrowInfo.BOARD_MARKET} ${escrowInfo.BOARD_SUBJECT}','${escrowInfo.BWID}')">물품대금 입금처리</button></c:if>
					...
					String p_link_url = "/index.do?curl=/escrow/system/trans_list.do";								// Link URL
